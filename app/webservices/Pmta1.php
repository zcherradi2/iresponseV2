        ...
        if($serverId > 0)
        {
            ...
            if($sshConnector->isConnected())
            {
                if($version == 'v5'){
                    ...
                    //$sshConnector->scp('send',array($pmta_host),'/etc/httpd/conf.d/powermta.conf'); 
                ...
                $result .= $sshConnector->cmd("cp /etc/pmta/config.rpmsave /home",true) . "\r\n";
                ...
                if($version == 'v5'){
                    ...
                    $sshConnector->cmd("rm -rf /etc/pmta/license-notice;");
                    $sshConnector->cmd("rm -rf /etc/pmta/license;");
                    $sshConnector->scp('send',array(ASSETS_PATH . DS . 'pmta/configs' . DS . 'license','/etc/pmta/license')); 
                }
                ...
                $result .= $sshConnector->cmd('service httpd restart;',true); 
