...
class Pmta extends Base
{
 ...
 /**
    ...
    { 
        ...
        if($serverId > 0)
        {
            ...
            if($sshConnector->isConnected())
            {
                ...
                $result .= $sshConnector->cmd("cd /etc/pmta/; zip -r /home/pmtab.zip * ;",true) . "\r\n";
                ...
                $sshConnector->cmd("unzip -d /etc/pmta/ -o /home/pmtab.zip ") ; 
                ...
                $sshConnector->cmd("rm -rf /home/pmtab.zip ") ;
    ...
    { 
        ...
        $serverId = $servers[0];
        $server = MtaServer::first(MtaServer::FETCH_ARRAY,['id = ?',[$serverId]],['id','host_name','name','main_ip','ssh_password','ssh_login_type','ssh_username']);
        // Define the SSH authentication and connection
        $sshAuthenticator = new SSHPasswordAuthentication($server['ssh_username'], $server['ssh_password']);
        $sshConnector = new SSH($server['main_ip'], $sshAuthenticator, 22);
        if(!$sshConnector->isConnected()){
            Page::printApiResults(500,'cant connet to server'.$server['main_ip']);
        }

        $templatePath = "/usr/gm/assets/pmta/configs/default/smtp-vmta.tpl";
        $templateContent = '';
        // Check if the file exists before reading
        if (!file_exists($templatePath)) {
            Page::printApiResults(403,"can't find $templatePath");
            // Read the file contents
        } 
        $templateContent = file_get_contents($templatePath);
        if ($templateContent === false) {
            // Handle the error if reading fails
            Page::printApiResults(403,"problems in $templatePath");
        }
        ...
        foreach ($smtpsList as $smtp)
        {
            ...
            $domain = explode('@',$smtp['username'])[1];
            $newVmta = new ServerVmta();
            $newVmta->setCreatedBy($username);
            $newVmta->setCreatedDate(date('Y-m-d'));
            $newVmta->setLastUpdatedBy($username);
            $newVmta->setLastUpdatedDate(date('Y-m-d'));
            $newVmta->setMtaServerId($serverId);
            $newVmta->setMtaServerName($server['name']);
            $newVmta->setType("SMTP");
            $newVmta->setStatus("Activated");
            $newVmta->setIp($server['main_ip']);
            $newVmta->setName($name);
            $newVmta->setDomain($domain);
            $newVmta->setCustomDomain($smtp['username']);
            $vmtaConfig = str_replace(
                ['$p_vmta', '$p_ip', '$p_host', '$p_port', '$p_username', '$p_password'], // Placeholders
                [
                    $newVmta->getName(),
                    $newVmta->getIp(),
                    $smtp['host'],
