...
/**
 * @name ScalewayAccounts
 * @description ScalewayAccounts Controller
 */
class ScalewayAccounts extends Controller
{
    /**
     * @app
     * @readwrite
    ...
    { 
        ...
        $this->masterView->set([
            ...
            'scaleway_management' => 'true',
            'scaleway_accounts' => 'true',
            'scaleway_accounts_show' => 'true'
        ]);
    ...
    { 
        ...
        if(count($data))
        {
            ...
            die(json_encode(DataTable::init($data,'admin.scaleway_accounts',$columns,new ScalewayAccount(),'scaleway-accounts','ASC')));
        }
    }
    ...
    { 
        ...
        $this->masterView->set([
            'cloud_management' => 'true',
            'scaleway_ocean_management' => 'true',
            'scaleway_accounts' => 'true',
            'scaleway_accounts_show' => 'true'
        ]);
        ...
        $account = ScalewayAccount::first(ScalewayAccount::FETCH_ARRAY,['id = ?',$id]);
        ...
        else
        {
            ...
            Page::registerMessage('error','Invalid scaleway account id !');
    ...
    { 
        ...
        {        
            ...
            $account->setName($this->app->utils->arrays->get($data,'scaleway-name'));
            $account->setStatus($this->app->utils->arrays->get($data,'scaleway-status'));
            $account->setApiToken($this->app->utils->arrays->get($data,'scaleway-api-token'));
            $account->setOrganization($this->app->utils->arrays->get($data,'scaleway-organization-id'));
            
            # check if there is a proxy involved
            if(filter_var($this->app->utils->arrays->get($data,'scaleway-proxy-ip'),FILTER_VALIDATE_IP))
            {
                $account->setProxyStatus('Enabled');
                $account->setProxyIp($this->app->utils->arrays->get($data,'scaleway-proxy-ip'));
                $account->setProxyPort($this->app->utils->arrays->get($data,'scaleway-proxy-port'));
                $account->setProxyUsername($this->app->utils->arrays->get($data,'scaleway-proxy-username'));
                $account->setProxyPassword($this->app->utils->arrays->get($data,'scaleway-proxy-password'));
            }
            else
            {
