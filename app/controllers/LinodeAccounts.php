...
/**
 * @name LinodeAccounts
 * @description LinodeAccounts Controller
 */
class LinodeAccounts extends Controller
{
    ...
    { 
        ...
        $this->masterView->set([
            ...
            'linode_management' => 'true',
            'linode_accounts' => 'true',
            'linode_accounts_show' => 'true'
        ]);
    ...
    { 
        ...
        if(count($data))
        {
            ...
            die(json_encode(DataTable::init($data,'admin.linode_accounts',$columns,new LinodeAccount(),'linode-accounts','ASC')));
        }
    }
    ...
    { 
        ...
        $account = LinodeAccount::first(LinodeAccount::FETCH_ARRAY,['id = ?',$id]);
    ...
    { 
        ...
        {        
            ...
            $account->setName($this->app->utils->arrays->get($data,'linode-name'));
            $account->setStatus($this->app->utils->arrays->get($data,'linode-status'));
            $account->setToken($this->app->utils->arrays->get($data,'linode-token'));
            
            # check if there is a proxy involved
            if(filter_var($this->app->utils->arrays->get($data,'linode-proxy-ip'),FILTER_VALIDATE_IP))
            {
                $account->setProxyStatus('Enabled');
                $account->setProxyIp($this->app->utils->arrays->get($data,'linode-proxy-ip'));
                $account->setProxyPort($this->app->utils->arrays->get($data,'linode-proxy-port'));
                $account->setProxyUsername($this->app->utils->arrays->get($data,'linode-proxy-username'));
                $account->setProxyPassword($this->app->utils->arrays->get($data,'linode-proxy-password'));
            }
            else
            {
